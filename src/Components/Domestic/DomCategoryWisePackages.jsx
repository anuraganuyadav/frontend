...
//                                 <Link to={`/packages/domestic/${country.name.toLowerCase()}`}> {/* Link to Packages with type and city */}
//                                     <img
//                                         src={country.image}
...
import Header2 from '../Common/Header2';
import Footer from '../Common/Footer';
import { city, categoryContent } from '../js/CityData';  // Import data from cityData.js

const DomCategoryWisePackages = () => {
    const { category } = useParams();
    const [content, setContent] = useState(categoryContent.honeymoon); // Default content for honeymoon packages

    useEffect(() => {
        // Switch content based on category
        if (category === 'honeymoon-packages') {
            setContent(categoryContent.honeymoon);
        } else if (category === 'family-packages') {
            setContent(categoryContent.family);
        } else if (category === 'group-packages') {
            setContent(categoryContent.group);
        } else if (category === 'weekend-packages') {
            setContent(categoryContent.weekend);
        } else if (category === 'solo-packages') {
            setContent(categoryContent.solo);
        }
    }, [category]);
    ...
    return (
        <>
            <Header1 />
            <Header2 />
            <div className="container-fluid bgbreadcrumb" style={{
                background: `linear-gradient(rgba(19, 53, 123, 0.5), rgba(19, 53, 123, 0.5)), url(${content.bannerImage})`,
                backgroundPosition: 'center', backgroundRepeat: 'no-repeat', backgroundSize: 'cover', backgroundAttachment: 'fixed', padding: '150px 0 50px 0'
            }}>
                <div className="container text-center py-5" style={{ maxWidth: '900px' }}>
                    <h3 className="text-white display-3 mb-4">{content.title}</h3>
                    <div className={styles.bookpackages}>
            ...
            <div className="container">
                ...
                <div className="row">
                    {city.map((country) => (
                        <div key={country.name} className="col-lg-3 col-md-4 col-sm-6 mb-4">
                            <div className={styles.countryCard}>
                                <Link to={`/packages/domestic/${country.name.toLowerCase()}`}>
                                    <img src={country.image} alt={`${country.name} landscape`} className={styles.countryImage} />
                                    <h3 className={styles.countryName}>{country.name}</h3>
                                </Link>
                            </div>
                        </div>
